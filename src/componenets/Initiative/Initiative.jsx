    ...
    <div className="eminent">
      ...
      <Container>
        <Row>
          {BureasData.map(
            ({ title, heading, profile, description, imgSrc }, index) => (
              <Col xs={12} md={6} lg={3} key={`bureaus-${index}`}>
                <Cards
                  title={title}
                  heading={heading}
                  profile={profile}
        ...
        <Row>
          {BureasData2.map(
            ({ title, heading, profile, description, imgSrc }, index) => (
              <Col xs={12} md={6} lg={3} key={`bureaus2-${index}`}>
                <Cards
                  title={title}
                  heading={heading}
                  profile={profile}
        ...
        <Row>
          {BureasData3.map(
            ({ title, heading, profile, description, imgSrc }, index) => (
              <div key={`$bureaus3-${index}`}>
                <Col style={{ paddingBottom: "3vw" }} s={12} md={5} lg={3}>
                  <img src={imgSrc} alt="img2" />
                </Col>
                <Col s={1} md={0} lg={1}></Col>
              </div>
            )
          )}
        </Row>
